...
`calc-work` under the directory `./calc` was started in. The key is the
problem id when it is made only of `[A-Za-z0-9._-]` (every problem id is),
else, and for your own goal, `goal-` and the first 12 hex digits of the
SHA-256 of the goal text and the sorted functions. A key never comes from
an imported file's name.

**When.** After every change to the session's saved content: an accepted
...
nodes that replayed. New node ids are assigned in order, so they equal the
old ones when nothing was dropped; the response carries the map.

**The checked prefix.** The page marks the first k sentences checked, where
k is the longest prefix of `script.sentences(script)` whose sentence k
parses (SCRIPT.md) to the same move and args as the k+1-th node of the
mapped `path`. So a file whose path and script drifted apart (a crash
...
inside `..`. A test runs both over the same scripts.
...
- `POST /session` takes `resume: bool` (default false, so existing
  ...
  `"resumed": {script, path, checked, dropped, ids, max_rung}` (`path`
  already mapped and cut to `checked + 1` nodes). With no file,
  `"resumed": null`. A file that is not a readable `calc-work` version 1
...
- The script is posted with `/script` after each step, retraction and
  ...
  so there is one saved copy of the work, the server's.
...
1. `app/test_work.py`: a session with a dead end (a retracted branch), a
   ...
   gives the matching checked prefix; a file holding a report or theorem
   field is read as moves only (the field is ignored); a corrupt file is
   moved aside; the write is atomic (the old file survives a failed
   write); `script.sentences` agrees with the page's `nextSentence`.
2. `app/test_api.py`: `/session` resume, `/script`, `/export`, `/import`
   and `/tree`'s `saved`, over a temporary work directory.
